...
bool EigenSystem(RealMatrix& t, ComplexVector& eigenvalues, ComplexMatrix& eigenvectors)
{
	...
	for(int pln = 0; pln < 3; pln++)
	{
		...
		b(2 * pln + 1) = b(2 * pln);
                ...
                for(int i=0;i<6;i++) for(int j=0;j<6;j++) Check(i)+=m(i,j)*b(j);
                double test=0;
                for(int i=0;i<6;i++) test+=pow(ABS(Check(i)-b(i)),2);
		Subscript n = 0;
		Subscript row = 0;
                if(test > 1.0E-15) { // if the gues is spot on, iteration fails! 
