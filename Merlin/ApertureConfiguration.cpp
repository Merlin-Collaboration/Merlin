				...
				&& ((*itr)->getSlongitudinal() >= Position || itr == (ApertureList.end() - 1)))
			{
				...
				for(size_t n = 1; n < ThisElementAperture.size(); n++)
				{
					if(ThisElementAperture[0]->getRectHalfWidth() != ThisElementAperture[n]->getRectHalfWidth()
						|| ThisElementAperture[0]->getRectHalfHeight() != ThisElementAperture[n]->getRectHalfHeight()
						|| ThisElementAperture[0]->getEllipHalfWidth() != ThisElementAperture[n]->getEllipHalfWidth()
						|| ThisElementAperture[0]->getEllipHalfHeight() != ThisElementAperture[n]->getEllipHalfHeight())
					{
						interpolate = true;
					}
				...
				if(interpolate)
				{
					...
					for(size_t n = 0; n < ThisElementAperture.size(); n++)
					{
						Aperture* apInterpolated = factory->getInstance(ThisElementAperture[n]->getType(),
							...
							ThisElementAperture[n]->getEllipHalfWidth(), ThisElementAperture[n]->getEllipHalfHeight());
