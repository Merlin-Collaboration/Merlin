...
public:
	...
	double interpolate(double s)
	{
		...
		int i1;
		if(index==0){i1=1;}
		else if(index==ncoeff-1) {i1=ncoeff-2;}
		else {i1=index;}
	...
	collimatortable(const char* file, double Gamma=0, double xi=0)
	{
		...
		if(!f)
		{
			cout << "Cannot open collimator table file: " << file <<endl;
			exit(EXIT_FAILURE);
		}
		...
		{ // interpolate xi value
			int index=int(0.5+xi/step1);
			if(index<0) index=0;
			int i1;
			if(index==0){i1=1;}
			else if(index==n1-1) {i1=n1-2;}
			else {i1=index;}
			for(int i=0; i<ncoeff; i++)
			{
				if(index>=n1) {coeff[i]=0;}
				else 
				{
		...
		{// 2D interpolation
			...
			int ig1;
			if(indg==0){ig1=1;}
			else if(indg==n1-1) {ig1=n1-2;}
			else {ig1=indg;}
			int indc=int(0.5+xi/step1);
			if(indc<0) indc=0;
			int ic1;
			if(indc==0){ic1=1;}
			else if(indc==n3-1) {ic1=n3-2;}
			else {ic1=indc;}
