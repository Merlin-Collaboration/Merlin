...
HollowElectronLens::HollowElectronLens (const string& id, double len, int mode, double current, double beta_e, double rigidity, double length_e)
	: Drift(id,len),
	  Current(current), ElectronBeta(beta_e), Rigidity(rigidity), EffectiveLength(length_e),
	  XOffset(0), YOffset(0),
