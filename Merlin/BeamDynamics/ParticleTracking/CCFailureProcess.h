...
// Process specific functions  
    ...
    virtual void SetFailureOnOff(bool onoff){failure_on = onoff; cout << "CCFailure::Failure_on = " << failure_on << endl;}
    
    // Select number of turns for failure
    virtual void SetFailureTurns(int ft){fail_turns = ft; cout << "CCFailure::SetFailureTurns = " << fail_turns << endl;}
    virtual void SetNonFailureTurns(int nft){non_fail_turns = nft; cout << "CCFailure::SetNonFailureTurns = " << non_fail_turns << endl;}
    
    // Select which plane to fail (horizontal = ATLAS, vertical = CMS)
    virtual void SetFailurePlanes(bool hor, bool ver){ATLAS_on = hor; CMS_on = ver;cout << "CCFailure::SetFailurePlanes: ATLAS = " << ATLAS_on << ", CMS = " << CMS_on << endl;}
