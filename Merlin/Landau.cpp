...
{
	static const float F[] = {0,0,0,0,0,0,
//6
...
//982
//End
	//See CERNLIB/CLHEP for what is going on here.
	//double U=1000.0 * pGenerator->asDouble();
	double r =  pGenerator->asDouble();
	double U = 1000.0 * r;
	int I = static_cast<int>(U);

	double du=U-static_cast<double>(I);
	if(I >= 70 && I <= 800)
	{
		return F[I] + du*(F[I+1]-F[I]);
	}
	else if(I >=  7 && I <= 980)
	{
		return F[I] + du*( F[I+1] - F[I]- 0.25*(1-du) * (F[I+2]-F[I+1]-F[I]+F[I-1]));
	}
	else if(I < 7)
	{
		double L=log(r);
	...
	else if(I <= 999)
	{
		double V = 1.0 - r;
		double VV = V*V;
		return (1.00060006 + 263.991156*V + 4373.20068*VV) / (V * (1.0 + 257.368075*V + 3414.48018*VV));
	}
	else
	{
		double V = 1.0 - r;
		double VV = V*V;
		return (1.00001538 + 6075.14119*V + 734266.409*VV) / (V * (1.0 + 6065.11919*V + 694021.044*VV));
	}
	/*
		{
			double U=1-X;
			...
			if(X <= 0.999)
			{
				return (1.00060006+2.63991156e2*U+4.37320068e3*V)/((1 + 2.57368075e2*U+3.41448018e3*V)*U);
			}
			else
			{
		  		return (1.00001538+6.07514119e3*U+7.34266409e5*V)/ ((1 + 6.06511919e3*U+6.94021044e5*V)*U);
			}
		}
	*/
}
